...
- ⬜ **Browser playground** — paste psql/health-score output, render the report fully client-side (no DB connection, no install).
- ⬜ **Health badge** — SVG `Postgres Health: A (87/100)` generated by the GitHub Action, droppable in any README.
- ⬜ **Docker one-liner** — `docker run pgvitals --url postgres://…`.
